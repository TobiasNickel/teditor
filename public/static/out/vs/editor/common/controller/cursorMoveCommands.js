    ...
    class CursorMoveCommands {
        ...
        static _moveLeft(viewModel, cursors, inSelectionMode, noOfColumns) {
            ...
            for (let i = 0, len = cursors.length; i < len; i++) {
                ...
                let newViewState = cursorMoveOperations_1.MoveOperations.moveLeft(viewModel.cursorConfig, viewModel, cursor.viewState, inSelectionMode, noOfColumns);
                if (skipWrappingPointStop && noOfColumns === 1 && newViewState.position.lineNumber !== cursor.viewState.position.lineNumber) {
                    ...
                    if (newViewModelPosition.lineNumber === cursor.modelState.position.lineNumber) {
                        ...
                        newViewState = cursorMoveOperations_1.MoveOperations.moveLeft(viewModel.cursorConfig, viewModel, newViewState, inSelectionMode, 1);
                    }
                }
        ...
        static _moveRight(viewModel, cursors, inSelectionMode, noOfColumns) {
            ...
            for (let i = 0, len = cursors.length; i < len; i++) {
                const cursor = cursors[i];
                const skipWrappingPointStop = hasMultipleCursors || !cursor.viewState.hasSelection();
                let newViewState = cursorMoveOperations_1.MoveOperations.moveRight(viewModel.cursorConfig, viewModel, cursor.viewState, inSelectionMode, noOfColumns);
                if (skipWrappingPointStop && noOfColumns === 1 && newViewState.position.lineNumber !== cursor.viewState.position.lineNumber) {
                    // moved over to the next view line
                    const newViewModelPosition = viewModel.coordinatesConverter.convertViewPositionToModelPosition(newViewState.position);
                    if (newViewModelPosition.lineNumber === cursor.modelState.position.lineNumber) {
                        // stayed on the same model line => pass wrapping point where 2 view positions map to a single model position
                        newViewState = cursorMoveOperations_1.MoveOperations.moveRight(viewModel.cursorConfig, viewModel, newViewState, inSelectionMode, 1);
                    }
                }
                result[i] = cursorCommon_1.CursorState.fromViewState(newViewState);
            }
...
//# __sourceMappingURL=cursorMoveCommands.js.map