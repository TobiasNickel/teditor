...
define(["require", "exports", "vs/base/common/event", "vs/workbench/common/editor", "vs/platform/instantiation/common/instantiation", "vs/platform/configuration/common/configuration", "vs/base/common/lifecycle", "vs/platform/registry/common/platform", "vs/base/common/arrays"], function (require, exports, event_1, editor_1, instantiation_1, configuration_1, lifecycle_1, platform_1, arrays_1) {
    ...
    let EditorGroup = class EditorGroup extends lifecycle_1.Disposable {
        constructor(labelOrSerializedGroup, instantiationService, configurationService) {
            ...
            //#region events
            this._onDidActivateEditor = this._register(new event_1.Emitter());
            this.onDidActivateEditor = this._onDidActivateEditor.event;
            this._onDidOpenEditor = this._register(new event_1.Emitter());
            this.onDidOpenEditor = this._onDidOpenEditor.event;
            this._onDidCloseEditor = this._register(new event_1.Emitter());
            this.onDidCloseEditor = this._onDidCloseEditor.event;
            this._onDidDisposeEditor = this._register(new event_1.Emitter());
            this.onDidDisposeEditor = this._onDidDisposeEditor.event;
            this._onDidChangeEditorDirty = this._register(new event_1.Emitter());
            this.onDidChangeEditorDirty = this._onDidChangeEditorDirty.event;
            this._onDidChangeEditorLabel = this._register(new event_1.Emitter());
            this.onDidEditorLabelChange = this._onDidChangeEditorLabel.event;
            this._onDidMoveEditor = this._register(new event_1.Emitter());
            this.onDidMoveEditor = this._onDidMoveEditor.event;
            this._onDidChangeEditorPinned = this._register(new event_1.Emitter());
            this.onDidChangeEditorPinned = this._onDidChangeEditorPinned.event;
            this._onDidChangeEditorSticky = this._register(new event_1.Emitter());
            this.onDidChangeEditorSticky = this._onDidChangeEditorSticky.event;
            this.editors = [];
            this.mru = [];
        ...
        getEditors(order, options) {
            const editors = order === 0 /* MOST_RECENTLY_ACTIVE */ ? this.mru.slice(0) : this.editors.slice(0);
        ...
        registerEditorListeners(editor) {
            ...
            listeners.add(event_1.Event.once(editor.onDispose)(() => {
        ...
        closeEditor(candidate, openNext = true) {
        ...
        doCloseEditor(candidate, openNext, replaced) {
        ...
        contains(candidate, options) {
            for (const editor of this.editors) {
                if (this.matches(editor, candidate, options === null || options === void 0 ? void 0 : options.strictEquals)) {
                    return true;
                }
                if ((options === null || options === void 0 ? void 0 : options.supportSideBySide) && editor instanceof editor_1.SideBySideEditorInput) {
                    if (this.matches(editor.primary, candidate, options === null || options === void 0 ? void 0 : options.strictEquals) || this.matches(editor.secondary, candidate, options === null || options === void 0 ? void 0 : options.strictEquals)) {
                        return true;
                    }
                }
            }
        ...
        matches(editor, candidate, strictEquals) {
            if (!editor || !candidate) {
                return false;
            }
            if (strictEquals) {
        ...
        deserialize(data) {
            ...
            this.editors = arrays_1.coalesce(data.editors.map((e, index) => {
            ...
            this.mru = arrays_1.coalesce(data.mru.map(i => this.editors[i]));
...
//# __sourceMappingURL=editorGroup.js.map